defmodule Autolaunch.XMTPMirror.Rooms do
  @moduledoc false
  ...
  @spec auction_room_key(String.t() | integer()) :: String.t()
  def auction_room_key(auction_id), do: "auction:#{room_ref!(auction_id)}"

  @spec token_room_key(String.t() | integer()) :: String.t()
  def token_room_key(token_id), do: "token:#{room_ref!(token_id)}"
  ...
  def public_room?(%XmtpRoom{room_key: room_key}) when is_binary(room_key) do
    String.starts_with?(room_key, "#{@canonical_room_key}-shard-") or
      String.starts_with?(room_key, "auction:") or
      String.starts_with?(room_key, "token:")
  end
  ...
  defp ensure_next_shard_room do
    ...
    if canonical_room do
      ...
      case ensure_room(%{
             ...
             "name" => "#{canonical_room.name || "Autolaunch Room"} ##{next_number}",
  ...
  defp room_ref!(value) when is_integer(value) and value > 0, do: Integer.to_string(value)

  defp room_ref!(value) when is_binary(value) do
    case String.trim(value) do
      "" -> raise ArgumentError, "room reference is required"
      trimmed -> trimmed
    end
  end
