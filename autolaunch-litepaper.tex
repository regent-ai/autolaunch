...
\usepackage{parskip}
\usepackage{tikz}
\usetikzlibrary{arrows.meta,calc,decorations.pathreplacing,positioning,shapes.geometric,shapes.symbols}
...
\graphicspath{{/Users/sean/Documents/regent/images/litepapers/autolaunch/}{../Documents/regent/images/litepapers/autolaunch/}}
...
That point matters because agent businesses spend in stable units. Compute, API usage, researchers, operators, reviewers, and infrastructure are not paid in vibes. A token may coordinate a community. A stablecoin treasury keeps the system alive.

This is why the recognized-revenue rule matters so much in Autolaunch. Only Base \USDC{} that reaches the subject revsplit counts as recognized revenue. That creates a hard line between narrative and measurable economic activity.
...
Autolaunch makes the most sense now because three things are finally true at the same time.
...
Second, wallet and stablecoin infrastructure are finally good enough to serve as operating rails rather than side experiments. Base flows and \USDC{} rails make it practical to talk about treasury, revenue, and settlement in the same environment where the product already lives.
...
On the contract side, it includes the machinery for launch deployment, liquidity formation, vesting, subject registration, revenue ingress, revsplit accounting, and a separate company-level \REGENT{} staking rail.
...
It also changes the dominant strategy. The design is meant to reward truthful demand more than tactical delay. Waiting does not magically improve your price path. It usually shortens your participation window.

For crypto-native readers, the easiest way to think about this is as a bounded, launch-window TWAP order embedded inside the primary market rather than a pure reflex race.
...
The equations below are schematic. They are meant to make the economic design legible, not to serve as line-by-line contract pseudocode.

Let bidder \(i\) choose a total budget \(B_i\) and a maximum acceptable token price \(\bar{p}_i\). Let \(p_t\) be the clearing price in block \(t\), and let \(x_{i,t}\) be the amount of the bidder's budget that clears in that block. Then the order can only clear while the market price is at or below the bidder's cap:

\begin{align}
x_{i,t} > 0 &\implies p_t \le \bar{p}_i, \\
\sum_{t=1}^{T} x_{i,t} &\le B_i.
\end{align}

If a spend amount \(x_{i,t}\) clears at price \(p_t\), then the token output for that block is

\begin{align}
q_{i,t} &= \frac{x_{i,t}}{p_t}, \\
Q_i &= \sum_{t=1}^{T} q_{i,t}.
\end{align}

A simple launch-window pacing rule can be written as

\begin{equation}
\hat{x}_{i,t} = \frac{B_i^{\mathrm{rem}}(t)}{N^{\mathrm{rem}}(t)},
\end{equation}

where \(B_i^{\mathrm{rem}}(t)\) is the bidder's remaining budget and \(N^{\mathrm{rem}}(t)\) is the number of blocks remaining in the launch window. The practical intuition is straightforward: the order is spread through time, bounded by a buyer-specified ceiling price, instead of collapsing into a one-second reflex race.

This does not make markets perfect. Large buyers still matter. Demand can still be wrong. External sentiment can still overwhelm careful design. But it does improve the starting conditions for price discovery, and that matters.
...
Let \(R\) denote recognized subject revenue deposited into the splitter in Base \USDC{}. Let \(\alpha = 0.01\) be the fixed platform skim. Then
...
\REGENT{} represents participation in the wider Regents system and its broader revenue rails.
...
Stablecoin revenues & Revsplit-tracked gross revenue. Regents Labs takes 1\% of gross revenue for all agents, from x402, MPP, and other sources, tracked onchain through the revsplit contract. \\
...
The broader point is that \REGENT{} is not meant to be a vague ecosystem badge. It is the company-level token that sits above the product stack and accumulates value from real fee rails across Autolaunch, Techtree, stablecoin revenue flows, and the hosted Regents platform.
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \item Claim your stablecoin share of Regents Labs revenue at any time.
  \item After the staker share is accounted for, the remaining balance is used to buy back \REGENT.
  \item During the first year, an emissions stream equal to 20\% of staked \REGENT{} is paid to stakers.
\end{enumerate}
...
where \(P\) is total protocol revenue for the accounting window and \(\gamma\) is the portion owed to stakers under the live staking state. The design intent is explicit: once the staker share is reserved, the remaining balance is used to buy back \REGENT.

At launch, only roughly 20\% of \REGENT{} is circulating. That means the staker-owed portion is expected to be relatively small in the earliest phase, so 80\% or more of protocol skim should flow toward buybacks rather than immediate claims.
...
\begin{itemize}[leftmargin=1.5em]
  \item stablecoin claims on Regents Labs revenue
  \item buyback pressure from the residual protocol balance
  \item a first-year emissions stream that rewards early conviction
\end{itemize}
...
The core claim is powerful because it is concrete.

A useful agent should be able to raise capital.

A serious launch should create stable runway, not only a chart.

A post-launch token should have a stronger reason to exist than pure momentum.

A real business should be able to route real stablecoin revenue into a visible onchain lane.

Autolaunch matters for more than launch mechanics because it shows how agent businesses can move from demo to company.
...
Recognized subject revenue rule & Only Base \USDC{} that reaches the subject revsplit counts as recognized revenue \\
