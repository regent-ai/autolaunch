...
\AddToShipoutPictureFG*{%
  \put(\LenToUnit{6.35in},\LenToUnit{9.95in}){%
    \includegraphics[width=0.9in]{autolaunch-title-logo.png}%
  }%
}
...
Autolaunch is built to solve the whole path, not just the launch moment. It gives an agent business a way to prepare a launch, run a continuous clearing auction, seed liquidity, create treasury runway in stable units, and route future \USDC{} revenue through onchain rails that remain legible after launch day.
...
This is why the agent revenue rule matters so much in Autolaunch. Base \USDC{} counts only after it reaches the agent-owned revsplit, and verified ingress, launch fees, and direct deposits are tracked separately. Base Sepolia is the rehearsal path where configured; Base mainnet is the production target. That creates a hard line between narrative and measurable economic activity.
...
Second, wallet and stablecoin infrastructure are good enough to serve as operating rails rather than side experiments. Base \USDC{} flows make it practical to talk about treasury, revenue, and settlement in the same environment where the product already lives.
...
In the Regents stack, the contracts use \textbf{subject} as the technical term for the agent business being launched, funded, and tracked. This paper uses the plain-English terms agent, agent business, or agent-owned lane except where the contract term matters.

Autolaunch includes a public market surface and a contract system underneath it.

On the product side, it includes public auction pages, launch preparation, agent pages, position views, trust follow-up, and an operator path that is explicitly CLI-first.

On the contract side, it includes the machinery for launch deployment, liquidity formation, vesting, agent registration, revenue ingress, revsplit accounting, and a separate company-level \REGENT{} staking rail. Ingress is the canonical receiving path that accepts raw \USDC{} and sweeps it into splitter accounting.
...
\begin{quote}
\textbf{Autolaunch is capital formation for agent businesses with an onchain path from launch to measurable \USDC{} revenue.}
\end{quote}
...
\midrule
Subject & A technical contract term for the agent business being launched, funded, tracked, and assigned a revenue lane. \\
Ingress & The receiving path that accepts eligible \USDC{} and sweeps it into splitter accounting. \\
Revsplit & The contract-defined split for agent-owned stablecoin inflows. \\
Safe & The agent's Safe or treasury wallet for operating funds. \\
...
MPP & Micropayment protocol-style revenue flows that can feed agent-owned stablecoin inflows when supported. \\
\bottomrule
\end{longtable}
...
The deployment flow creates the launch-side contract stack for the agent business. That includes the token, the auction strategy, the vesting path, fee plumbing, registration, and default revenue-ingress configuration.
...
After launch, the agent continues to exist as an operating business with a public market and a revenue path. Holders can follow the agent, stake when relevant, claim when relevant, and observe future \USDC{} revenue as it reaches the contract-defined lane.
...
The equations below are schematic. They make the economic design easier to read.
...
If the only treasury outcome from launch were more volatile token inventory, the business would still face a mismatch between operating costs and treasury assets. Autolaunch solves this by routing a meaningful share of the raise into \USDC{} runway for the agent Safe.
...
The launch creates an agent business with a market, a registry record, and a post-launch surface. The project does not disappear into a contract address. It continues as something people can follow, inspect, and act on.

\subsection{A route for real revenue}

The deeper product thesis appears after the launch. Once the agent begins to earn stablecoin revenue and that revenue reaches the defined revsplit lane, the system can route participation in a way that remains visible and measurable onchain.
...
Again, the equations below are schematic. They capture the economic intent of the revsplit rather than every implementation detail.

Let \(R\) denote agent-owned \USDC{} deposited into the splitter from the tracked source lanes. Let \(\alpha = 0.01\) be the fixed platform skim. Then

\begin{align}
F_{\text{Regents}}^{\text{rev}} &= \alpha R = 0.01R, \\
D_{\text{agent}} &= (1-\alpha)R = 0.99R.
\end{align}

Let \(\beta \in [0,1]\) denote the share of the agent-owned lane allocated to stakers for a given accounting window. The remainder stays with the agent treasury. Then

\begin{align}
D_{\text{stakers}} &= \beta D_{\text{agent}}, \\
D_{\text{treasury}} &= (1-\beta) D_{\text{agent}}.
\end{align}
...
The point of writing the revsplit this way is to show what changes once revenue arrives: the token stops being only a symbol of launch-day enthusiasm and becomes attached to a measurable economic lane.
...
The contract system owns the harder economic boundaries: supply creation, auction behavior, vesting, fee lanes, agent registration, \USDC{} ingress, and revenue splitting.
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \item each launched agent has a canonical registry and revenue lane
  \item \USDC{} revenue enters a contract-defined split rather than a vague offchain promise
\end{enumerate}
...
\section{Two Economic Layers: Agent Tokens and \texorpdfstring{\REGENT}{\$REGENT}}

Autolaunch includes two distinct economic layers, and keeping them separate is a feature, not a bug.

The first layer is the \textbf{agent token}. This is the token for one launched agent business. It is tied to that agent's launch, its revenue lane, and its holder or staker relationship.

The second layer is \textbf{\REGENT}, the wider company token for the Regents platform.

These are not the same thing.

An agent token represents participation in the economics of one launched agent business.

\REGENT{} represents participation in the wider Regents system and its broader protocol-defined revenue rails.

This separation matters for clarity. It means Regents does not have to flatten every launched agent into one company-wide pool, and it means a strong launched agent can have its own economics without pretending to be the entire platform.

For public readers, the important takeaway is straightforward: Autolaunch supports both agent-level markets and a distinct platform-level token rail, without confusing one for the other.
...
Regents Platform & Hosted agent margin fees. This includes OpenClaw and Hermes agent hosting, with Stripe LLM billing for margin fees on hosted Regents.\footnote{\url{https://docs.stripe.com/billing/token-billing}} \\
\bottomrule
\end{tabularx}
\end{table}
...
Fourth, the team is willing to operate in public. In the full Regents stack, the best version of an Autolaunch launch is paired with visible work, visible proof, or visible progress.
...
The first limit is that this is still a beta design. The architecture is real. The product intent is clear. But proving the full thesis requires live launches, real operators, and real \USDC{} revenue.

The second limit is that better market structure does not create perfect markets. A continuous clearing auction can reduce timing advantages without eliminating concentrated demand, poor assumptions, or bad pricing.
...
The fifth limit is regulatory and market uncertainty. Any system that combines token launches, onchain revenue routing, treasury mechanics, and public market participation should expect scrutiny.

These constraints define the proof Autolaunch must keep earning: useful launches, measurable stablecoin revenue, clear public proof, and simple communication between agent-token economics and \REGENT{} economics.
...
Treasury split at launch & Half of auction \USDC{} to LP, half of auction \USDC{} to the agent Safe \\
Retained supply & Remaining 85\% vests to the agent treasury over one year \\
Official launch pool fee & 2\% \\
Launch fee split & 1\% to Regents Labs, 1\% to the agent-owned revenue lane \\
CCA raise fee & 2\% of \USDC{} raised in the continuous clearing auction \\
Agent \USDC{} rule & Base \USDC{} counts only after it reaches the agent-owned revsplit; verified ingress, launch fees, and direct deposits are tracked separately \\
Agent \USDC{} split & Fixed 1\% skim to Regents Labs, 99\% remains in the agent-owned lane \\
\bottomrule
\end{tabularx}
\end{table}
